...
INA226 dbInputPower(I2Cinternal, 0x41);
INA226 dbDCDCPower(I2Cinternal, 0x42);
...
// Radio Object
LOBEpRadio lobepRadio(lineTX, lineRX, controlSPI, tx, rx);

// services running in the system
HousekeepingService<LOBEpTelemetryContainer> hk;
TestService test;
PingService ping;
...
LOBEpService lobepService(lobepRadio);


//SoftwareUpdateService SWUpdate;
Service* services[] = {&lobepService, &hk, &ping, &reset, &test, &SWupdate };//{&radioService, &hk, &ping, &reset, &SWUpdate, &tst };
...
Task* tasks[] = { &cmdHandler, &timerTask, &lobepRadio};
...
void acquireTelemetry(LOBEpTelemetryContainer *tc)
{
    ...
    tc->setTemperature(t);
    tc->setTransmitINAStatus(!(dbInputPower.getVoltage(v)) & !(dbInputPower.getCurrent(i)));
...
//    tc->setPhasingTemperature(t);
    ...
    //PABoard
    tc->setAmplifierINAStatus(!(dbDCDCPower.getVoltage(v)) & !(dbDCDCPower.getCurrent(i)));
...
void main(void)
{
    ...
    Console::log("LOBEp booting...SLOT: %d", (int) Bootloader::getCurrentSlot());
